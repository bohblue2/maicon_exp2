...
def get_hash(paths):
    ...
    size = sum(os.path.getsize(p) for p in paths[:5] if os.path.exists(p))  # sizes
...
def extract_boxes(path=DATASETS_DIR / 'coco128'):  # from yolov5.utils.dataloaders import *; extract_boxes()
