...
def convert_coco(img_path, save_dir, coco, is_train=True):
    ...
    folder_group = img_save_path.split("_")[-2]
    base_name = osp.basename(img_path)
    if is_train:
        upper_folder_group =  '_'.join([k for i, k in enumerate(base_name.split('_')) if (i != 6 and i != 7)])
        upper_folder_group = upper_folder_group.replace('.jpg', '')
        label_path = f'/mnt/d/ts/{upper_folder_group}/{folder_group}/사건사고데이터이미지_{base_name}'
        label_path = label_path.replace("/ts/", "/tl/").replace(".jpg", ".json")
    else:
        upper_folder_group =  '_'.join([k for i, k in enumerate(base_name.split('_')) if (i != 7 and i != 8)])
        upper_folder_group = upper_folder_group.replace('.jpg', '')
        upper_folder_group = upper_folder_group.replace('사건사고데이터이미지_', '')
        label_path = f'/mnt/d/vs/{upper_folder_group}/{folder_group}/{base_name}'
        label_path = label_path.replace("/vs/", "/vl/").replace(".jpg", ".json")
    try:
...
# train_coco = Coco()
# train_num = 100_000
# train_imgs = sorted(glob('/mnt/e/yolov5pip_ir/train' + f"/*_{carmera_type_prefix}_*"))
# train_ds = train_imgs
...
train_imgs = sorted(glob('/mnt/e/yolov5pip_ir/valid' + f"/*_{carmera_type_prefix}_*"))
valid_ds = valid_imgs
